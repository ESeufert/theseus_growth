...
def combine_DAU( DAU_totals, labels = None ):
    ...
    #sort the columns
    columns = sorted( [ int( c ) for c in combined_DAU.columns if c not in [ 'DAU', 'profile', 'cohort_date', 'age', 'Value' ] ] )
...
def project_cohorted_DAU( profile, periods, cohorts, DAU_target = None, 
    ...
    if DAU_target != None:
        forward_DAU = project_targeted_DAU( profile, forward_DAU, periods, cohorts, DAU_target, DAU_target_timeline, start_date )
